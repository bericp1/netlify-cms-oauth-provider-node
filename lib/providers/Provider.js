...
class Provider {
     ...
     * @return string
     */
    generateAuthorizeUri(state = null) {
        return this.oauth2.authorizationCode.authorizeURL({
            redirect_uri: this.config.get('completeUri'),
            scope: this.config.get('oauthScopes') || this.getDefaultScopes(),
            state: state || randomstring.generate(32),
            ...this.getAdditionalAuthorizeUriParameters(state),
        });
    }
     ...
     * @return {object}
     */
    getAdditionalAuthorizeUriParameters(state) {
        return {};
    }
