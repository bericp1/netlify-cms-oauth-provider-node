 ...
 */
const createBeginHandler = (...createArgs) => {
    return handlesErrors(async (req, res) => {
        ...
        const begin = createGenericBeginHandler(...createArgs);
 ...
 * @type {function(object=, CreateConfigOptions): AsyncNowServerlessFunction}
 */
const createCompleteHandler = (...args) => {
    return handlesErrors(async (req, res) => {
        const debug = createDebug(DEBUG_KEY, 'complete');
        // Create the generic handler inside our wrapped function to handle any config/initialization errors
        debug('Creating generic complete handler which this handler wraps...');
        const complete = createGenericCompleteHandler(...args);
